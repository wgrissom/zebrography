...
pmax = 1.65E6; % Pascal; peak pressure, at 200mV
...
p = real(P3d);
p = p./max(p(:))*pmax; %%%pmap_single = pmap_single./max(pmap_single(:))*pmax;
...
imagesm = imagesm / max(imagesm(:));
[imgs, epsilonPix, d]= calcImgMC(pInt,Zd/dx*(k*dn0dp)/n0, imagesm);
%% 
